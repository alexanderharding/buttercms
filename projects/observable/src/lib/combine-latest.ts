...
export function combineLatest<
	const Inputs extends Readonly<Record<never, never>>,
>(inputs: Inputs): Observable<never>;
export function combineLatest<
...
export function combineLatest(
	...
	return new Observable((subscriber) => {
		...
		// The loop to kick off subscription. We're keying everything with a symbol to relate the observables passed
		// in to the slot in the output array or the key in the array of keys in the output dictionary.
		normalizedInputs.forEach((input, i) => {
			from(input).subscribe({ ...subscriber, next, complete });

			function next(value: unknown): void {
				const hasFirstValue = i in values;
